...
if __name__ == '__main__':
    ...
    if not os.path.exists("divide_baseline_try/"+ file_name + "_two" + "/" + str(num_experiment)):
        os.makedirs("divide_baseline_try/"+ file_name + "_two" + "/" + str(num_experiment))
    log_df.to_csv("divide_baseline_try/"+ file_name + "_two" + "/" + str(num_experiment)+"/log.csv")

    if not os.path.exists("divide_baseline_try/"+ file_name + "_two" + "/" + str(num_experiment)):
        os.makedirs("divide_baseline_try/"+ file_name + "_two" + "/" + str(num_experiment))
    solution_df.to_csv("divide_baseline_try/"+ file_name + "_two" + "/" + str(num_experiment)+"/solution.csv")
    ...
    #     print(len(total))
    #     total_case = []
    #     for case_item in total:
    #         total_case.append(case_item)
    #     sample_total_list = divide_small(total_case, sample_size)
    #     # result = open("Experiment/result_SA_divid/+"+file_name+"+_sample_" + str(nn) + ".txt", "w")
    #     # for i in range(len(sample_total_list)):
    #     #     result.write(str(sample_total_list[i]))
    #     #     result.write("\n")
    #     #     for j in range(len(sample_total_list[i])):
    #     #         try:
    #     #             list_data.remove(sample_total_list[i][j])
    #     #         except:
    #     #             continue
    #     # result.write('\n')
    #     # result.write(str(list_data))
    #     # result.write("\n")
    #     sample_first_list, energy_first_list = run_qpu(sample_total_list, data, sample_time, sample_size, per)
    #     merge_sample_sel= merge(sample_first_list)
    #     fval = print_diet(merge_sample_sel, data, nn)
    #     scatter_merge(merge_sample_sel, data, nn)
    #     selected = []
    #     for case in merge_sample_sel.keys():
    #         selected.append(int(case[1:]))
    #     for i in range(len(selected)):
    #         total.remove(selected[i])
    #     sample_total_list = divide_small(total, sample_size)
    #     sample_first_list, energy_first_list = run_qpu(sample_total_list, data, sample_time, sample_size, per)
    #     merge_sample_unsel = merge(sample_first_list)
    #     fval = print_diet(merge_sample_unsel, data, nn)
    #     scatter_merge(merge_sample_unsel, data, nn)
    #     merge_total = dict(merge_sample_sel, **merge_sample_unsel)
    #     fval = print_diet(merge_total, data, nn)
    #     scatter_merge(merge_total, data, nn)
    #     unselected = []
    #     for case in merge_sample_unsel.keys():
    #         unselected.append(int(case[1:]))
    #     total = selected+unselected
    #     total = sorted(total)


    # result.write(str(fval))
    ...
    # # print(divide(data, sample_size))
    # chain = 100
    # # sample_total_list = bootstrap_sampling(data, sample_time, sample_size)
    # sample_total_list = divide(data, sample_size)
    # sample_first_list, energy_first_list = run_qpu(sample_total_list, data, sample_time, sample_size, per)
    # print("energy:"+str(energy_first_list))
    # merge_sample = merge(sample_first_list)
    # nn = 1
    # print_diet(sample_first_list, data,nn)
    # scatter_merge(sample_first_list, data)
    # print_diet(merge_sample, data)
