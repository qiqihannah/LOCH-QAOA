...
if __name__ == '__main__':
    # file_name = "elevator"
    # df = pd.DataFrame()
    # df = pd.read_csv("/Users/xinyi/PycharmProjects/TCO_ex3/data/"+file_name+".csv", dtype={"cost": float, "pcount": float, "dist":float})
    # best_solution, best_energy = get_initial_fval(len(df))
    # print(OrderByImpactNum(best_solution, df, best_energy))
    # print(OrderByImpact(best_solution, df, best_energy))
    ...
    if not os.path.exists("qaoa_"+str(reps)+"/" + file_name + "_one" + "/size_" + str(problem_size) + "/" + str(num_experiment)):
        os.makedirs("qaoa_"+str(reps)+"/" + file_name + "_one"+ "/size_" + str(problem_size) + "/" + str(num_experiment))
    log_df.to_csv("qaoa_"+str(reps)+"/" + file_name + "_one" + "/size_" + str(problem_size) + "/" + str(num_experiment)+"/log.csv")

    if not os.path.exists("qaoa_"+str(reps)+"/" + file_name + "_one" + "/size_" + str(problem_size) + "/" + str(num_experiment)):
        os.makedirs("qaoa_"+str(reps)+"/" + file_name + "_one" + "/size_" + str(problem_size) + "/" + str(num_experiment))
    result_df.to_csv("qaoa_"+str(reps)+"/" + file_name + "_one" + "/size_" + str(problem_size) + "/" + str(num_experiment)+"/itr_results.csv")

    if not os.path.exists("qaoa_"+str(reps)+"/" + file_name + "_one" + "/size_" + str(problem_size) + "/" + str(num_experiment)):
        os.makedirs("qaoa_"+str(reps)+"/" + file_name + "_one" + "/size_" + str(problem_size) + "/" + str(num_experiment))
    solution_df.to_csv("qaoa_"+str(reps)+"/" + file_name + "_one" + "/size_" + str(problem_size) + "/" + str(num_experiment)+"/solution.csv")
